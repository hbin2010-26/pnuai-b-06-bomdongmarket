...
- 연 4회 구매 기준의 월 환산 모종비와 순환식 손실을 반영한 양액비 계산
...
실행할 때마다 `output/Profit_Output.xlsx`도 새로 생성됩니다. Excel의 `요약` 시트에는 사용가능 바닥면적과 월 기기 대여비가 표시되며, `비용구성` 시트에는 전기비·수도비·재료비·인건비·기기 대여비·감가상각 등 기타비의 금액과 월 운영비 대비 비율이 표시됩니다. 같은 공간의 여섯 작물은 동시에 운영되는 합계가 아니라 서로 대체 가능한 비교안이므로, 콘솔의 18개 시나리오 합계는 비교 참고용입니다.
...
월 양액량은 실제 총 재배면적과 작물별 일일 증발산량에 30일 및 순환식 손실·보충 보정계수 `1.1`을 적용합니다.

```math
NutrientVolume_L
=
A_{grow}\times ET_{daily}\times30\times1.1
```
...
`crop_production_info.csv`의 기존 기타 월 재료비 `300,000원` 값은 보존하지만, 현재 총 재료비에는 중복 반영하지 않습니다.
...
- 양액 단가는 `20원/L`이며, 월 양액량은 실제 총 재배면적과 작물별 일일 증발산량을 기준으로 계산합니다.
