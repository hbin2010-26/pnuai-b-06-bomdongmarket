...
//
// 재배가능비율·층수·천장고는 설비 사양이라 실측이 없으면 정할 수 없다(#99). 사용자가 아는 값이
...
public record SpaceInputs(
    ...
    public static final int MIN_MODULE_LAYERS = 1;
    public static final int MAX_MODULE_LAYERS = 10;
    ...
    public static SpaceInputs of(double totalAreaM2, double desiredMonthlyRentKrw,
                                Double cultivableRatio, Integer moduleLayers, Double ceilingHeightM) {
        return new SpaceInputs(
                totalAreaM2,
                cultivableRatio != null ? cultivableRatio : DEFAULT_CULTIVABLE_RATIO,
                moduleLayers != null ? moduleLayers : DEFAULT_MODULE_LAYERS,
    ...
    public boolean usesDefaultFacilityAssumptions() {
        return cultivableRatio == DEFAULT_CULTIVABLE_RATIO
                && moduleLayers == DEFAULT_MODULE_LAYERS
